    ...
    for folders in mgName:
        if folders.startswith(("Raw", "Recon")):
            ...
            if len(plyfile) != 0:
                ...
                ixPath = filepath.replace('\\', '/')  # for Windows!!
            if "Skull" in folders:
            ...
            ixPath = filepath.replace('\\', '/') # for Windows!!
    return mgName, uf, ixPath, rawZsplit, reconZsplit
...
def createyml(fname, uf, ixPath, gbifl1, roiName):
    # print("fname: " + fname)
    ...
    if "dice" in fname:
        ...
        yfiles = ("    :files:\n%s\n%s\n      - /deepbluedata-prep/UMMZ/mammals/scripts/DBD_UMMZ_DiceCT_README_File.txt\n" % (
            ixPath, yUAFiles))  # pull from directory
    else:
                ...
                if fname.is_dir():
                    ...
                    if gbiflist == None:
                        ...
                        if ummzdict['ic'] == "error" or ummzdict['cn'] == "error":
                            ...
                            mgName, uf, ixPath, rawZsplit, reconZsplit = getmediagroup(serverpath, str(datafolder), fname.name, ummzdict['ic'], ummzdict['cc'], ummzdict['cn'], ummzdict['yuuid'])  # Get media groups zip media group folders
                            ...
                            createyml(fname.name, uf, ixPath, gbiflist[1], roiName)
        csv_file.close()
